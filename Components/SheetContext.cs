...
public class SheetContext
{
    ...
    public void UpdateHeader(bool isColumn, int index, string value)
    {
        if (isColumn && index >= 0 && index < Model.ColumnHeaders.Count)
        {
            Model.ColumnHeaders[index] = value;
        }
        else if (!isColumn && index >= 0 && index < Model.RowHeaders.Count)
        {
            Model.RowHeaders[index] = value;
        }

        NotifyStateChanged();
    }

    public void ApplyDragFill(CellRegion source, CellRegion target)
    {
        ...
        for (int tr = target.StartRow; tr <= target.EndRow; tr++)
        {
            for (int tc = target.StartCol; tc <= target.EndCol; tc++)
            {
                ...
                {
                    Model.SetCell(tr, tc, new CellValue(sourceValue.Value)
                    {
                        Format = sourceValue.Format,
                        BackgroundColor = sourceValue.BackgroundColor,
                        TextColor = sourceValue.TextColor
                    });
                }
                else
                {
